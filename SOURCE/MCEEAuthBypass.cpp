...
int main(int argc, char * argv[]) {
  ...
  printf_s("Сделано котиками из Cats Coding Batch.\n");
  FILE * ptr_file;
  ...
  char * tmp;
  #ifdef _WIN64
  printf_s("Версия x64 может использоваться ТОЛЬКО для 64-битных версий игры.\n");
  #else
  printf_s("Версия x86 может использоваться ТОЛЬКО для 32-битных версий игры.\n");
  #endif
  ...
  if ((_access(MEE_POINTER_FILE, 0)) != -1) // If the mee.ptr file exists AND the process has permission to read it..
  {
    ...
    */
    ...
    printf_s("Успешно загружен файл %s!\n", MEE_POINTER_FILE);
  } else // If no mee.ptr file is found ... OR the process dont have permission to read it
  {
    ...
    printf_s("Ошибка загрузки файла, используем pointer path поумалчанию (MCEE 1.17.30 UWP x64)\n");
    ...
    printf_s("Ошибка загрузки файла, используем pointer path поумалчанию (MCEE 1.17.30 Win32 x86)\n");
    // Use a default pointer path.
  ...
  DWORD proc_id = 0;
  printf_s("\nОткройте Minecraft Education Edition\n");
  ...
  if (!hProcess) // If failed to open the process (eg no permission)
  {
    ...
    printf_s("MCEE Основной Адресс: 0x%p\n", (void * ) baseAddress);
    printf_s("Ждем загрузки игры....\n");
    ...
    if (login_step_value != 0x0) {
      ...
      printf_s("Текущий этап логина: %i\n", login_step_value);
      if (LOGIN_STEP_VALUE != -1) {
        printf_s("Пробуем этап логина: %i", LOGIN_STEP_VALUE);
        WriteProcessMemory(hProcess, (LPVOID) ptr, & LOGIN_STEP_VALUE, sizeof(int), 0);
        goto finish;
      }

      printf_s("Пробуем этап логина: 5...\n"); // Backwards Comp (0.xx)
      int login_step_value = 5;
      WriteProcessMemory(hProcess, (LPVOID) ptr, & login_step_value, sizeof(int), 0);

      Sleep(1 * 200);

      printf_s("Пробуем этап логина: 6...\n"); // Backwards Comp (1.9 and lower)
      login_step_value = 6;
      WriteProcessMemory(hProcess, (LPVOID) ptr, & login_step_value, sizeof(int), 0);

      Sleep(1 * 200);

      printf_s("Пробуем этап логина: 8...\n");
      login_step_value = 8;
      WriteProcessMemory(hProcess, (LPVOID) ptr, & login_step_value, sizeof(int), 0);
